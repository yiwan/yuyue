...
Page({
			...
			registerShow: function(e) {
				...
				if(that.data.userID == undefined  ){  // 我只判断了一个 剩下的必填项你自己加 && 
					wx.showToast({
						title:"受访人不能为空",
						icon:"none"
					})
				}else if(that.data.time>=that.data.end_time){
					wx.showToast({
						title:"请选择正确时间",
						icon:"none"
					})
				}else if(that.data.end_time==="" && that.data.time ===""){
					wx.showToast({
						title:"请选择正确时间",
						icon:"none"
					})
